...
* @version $Id: papaya_calendar.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class papaya_calendar extends base_calendar {
  ...
  */
   ...
   * @var array
   */
  public $surferList = NULL;

  /**
   * @var array
   */
  public $loadedRegdate = NULL;
   ...
   * @var array
   */
  public $cacheDates = array();

  /**
   * @var array
   */
  public $regdateIgnores = array();

  /**
   * @var connector_surfers
   */
  public $surfersObj = NULL;

  /**
   * @var array
   */
  public $surferGroups = NULL;
  ...
  */
  function initialize($paramName = 'cal', $userId = '') {
    ...
    $this->localImages = array(
      ...
      'dates-spawn' => $this->papaya()->images['actions-database-refresh'],
      'dates-table' => $this->papaya()->images['items-table']
    );
  ...
  */
   ...
   *
   * @param int $permId optional, default value NULL
   * @param bool $errorMsg
   * @access public
   * @return boolean
   */
        ...
        if ($this->addRegdateTranslation()) {
          $this->addMsg(
            ...
            sprintf(
              ...
              papaya_strings::escapeHTMLChars($this->papaya()->administrationLanguage->id),
  ...
  */
  ...
  function loadRegdate($regdateId) {
    ...
    $params = array(
      $this->tableRegdates,
      $this->tableRegdateTrans, $this->papaya()->administrationLanguage->id,
  ...
  function recreateRegdateCache() {
    if (isset($this->loadedRegdate) && is_array($this->loadedRegdate)) {
      ...
      if (FALSE !== $deleted &&
        ...
        /** @var base_crontime $module */
        $module = $this->papaya()->plugins->get(
        ...
        if (@is_array($daysData) && (count($daysData) > 0)) {
          ...
          foreach ($daysData as $oneDay) {
            if ($id = $this->databaseInsertRecord($this->tableDates, 'date_id', $oneDay)) {
        ...
        if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          ...
          if ($date > 0) {
            ...
            if ($createTopic) {
              ...
              $result = $this->databaseDeleteRecord($this->tableDates, 'date_id', (int)$ignoreId);
            }
            if ($result) {
              $data = array(
              ...
              if ($this->databaseInsertRecord($this->tableRegignore, 'regdate_ignoreid', $data)) {
                $result = TRUE;
              }
            }
          }
  ...
  * Get next execute
  *
  * @param base_crontime $cronModule
  ...
  */
  function collaborateSurferAdmin() {
    if (!isset($this->surfersObj)) {
      ...
      $this->surfersObj = $this->papaya()->plugins->get(
  ...
  */
  function showOwnerEdit($data, $type) {
    ...
    $dialog->dialogTitle = $this->_gt('Owner');
  ...
  */
  function getComboLanguages($paramName, $field, $value) {
    ...
    foreach ($this->papaya()->languages as $language) {
      $selected = ($language['id'] == $value) ? ' selected="selected"' : '';
      $result .= sprintf(
  ...
  */
  function checkDateInput($data) {
    $result = TRUE;
    if (!PapayaFilterFactory::isNotXml($data['date_title'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
    ...
    if (!PapayaFilterFactory::isNotXml($data['date_text'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::matches($data['date_startf'], '/^\d{4}\-\d{2}\-\d{2}$/', TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::matches($data['date_endf'], '/^\d{4}\-\d{2}\-\d{2}$/', TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::matches($data['date_content_guid'], '/^[a-fA-F\d]{32}$/', TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
  ...
  */
  function checkRegdateInput() {
    ...
    $caclDates = FALSE;
    $updateDates = FALSE;
    if (!PapayaFilterFactory::isNotXml($this->params['regdate_title'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::isText($this->params['regdate_text'], FALSE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
      ...
      $result = FALSE;
    } elseif (!PapayaFilterFactory::isNotEmpty($this->params['regdate_text'])) {
      $this->addMsg(
        MSG_WARNING,
        sprintf(
    ...
    if (!PapayaFilterFactory::isInteger($this->params['regdate_days'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::isIsoDate($this->params['regdate_start_iso'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::isIsoDate($this->params['regdate_end_iso'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::isGuid($this->params['regdate_module_guid'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (!PapayaFilterFactory::isInteger($this->params['regdate_max'], TRUE)) {
      $this->addMsg(
        MSG_ERROR,
        sprintf(
          $this->_gt('The input in field "%s" is not correct.'),
    ...
    if (@is_array($this->cacheDates)) {
      ...
      foreach ($this->cacheDates as $id => $row) {
        if ($row['date_start'] < $row['date_end']) {
          $titleDate = date('Y-m-d', $row['date_start']).'-'.date('Y-m-d', $row['date_end']);
        } else {
          $titleDate = $this->_gt($this->wDay2Str($this->getWDay($row['date_start']))).
  ...
  */
    ...
    if ($isRegular && @is_array($this->loadedRegdate)) {
      if ($this->loadedRegdate['regdate_content_guid']) {
        $content = (strpos($this->loadedRegdate['regdate_data'], '<data') === 0)
          ? $this->loadedRegdate['regdate_data'] : sprintf(
   ...
   * @return string
   */
  function getModuleEdit($guid, $data, $saveFunc) {
    if ($guid) {
