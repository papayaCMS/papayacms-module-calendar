...
* @version $Id: base_calendar.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
class base_calendar extends base_db {
  ...
  * @access public
  */
  function initialize($paramName = 'cal') {
    $this->initLanguageId();
  ...
  */
  function initLanguageId() {
    if (!isset($this->currentLanguageId)) {
      $this->currentLanguageId = $this->papaya()->request->languageId;
    }
  }
  ...
  */
  function buildLink(
    $caption, $url = '', $params = NULL, $target = '', $class = '', $title = '', $htmlTag = TRUE
  ) {
    if ($url == '') {
      $url = $this->baseLink;
    }
      ...
      foreach ($this->dates as $row) {
        ...
        if (isset($this->contentModules[$row['date_content_guid']])) {
          ...
          $moduleObject = $this->papaya()->plugins->get(
            $module['module_guid'], $this, $content
          );
  ...
  */
  function checkInput($data) {
    ...
    if (!PapayaFilterFactory::isNotXml($data['date_data'], TRUE)) {
